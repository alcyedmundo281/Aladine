// script.js - VERSIÓN FINAL CORREGIDA Y REORDENADA

// =================================================================================
// 1. DEFINICIÓN DE DATOS GLOBALES
// =================================================================================
// Se definen primero para asegurar que existan cuando las funciones los necesiten.
...
async function generateProtocol() {
    console.log("Botón presionado. Función generateProtocol iniciada.");
    ...
    try {
        ...
        if (!responseData.candidates || responseData.candidates[0].finishReason === 'SAFETY') {
             throw new Error("La respuesta de la API fue bloqueada por políticas de seguridad de Google. Intente con un título diferente.");
        }
...
function renderProtocol(data) {
    ...
    let html = `<div class="protocol-header"><h1>PROTOCOLO: ${data.metadata.titulo}</h1><p><strong>Código:</strong> ${data.metadata.protocoloCodigo || 'HECAM-XX-PR-XXX'}</p><p><strong>Versión:</strong> ${data.metadata.version} | <strong>Unidad Responsable:</strong> ${data.metadata.unidadResponsable.nombre}</p><p><strong>Fecha de Elaboración:</strong> ${data.metadata.fechaElaboracion}</p></div><hr>`;
    Object.values(data.secciones).forEach(section => {
        ...
        if (section.contenido) {
            ...
            if (section.contenido.resultadosEsperados) { html += `<strong>Resultados Esperados:</strong><ul>${section.contenido.resultadosEsperados.map(item => `<li>${item}</li>`).join('')}</ul>`; }
        }
        if (section.general) html += `<p><strong>Objetivo General:</strong> ${section.general}</p>`;
        if (section.especificos) { html += `<strong>Objetivos Específicos:</strong><ul>${section.especificos.map(obj => `<li>${obj}</li>`).join('')}</ul>`; }
        if (section.terminos) { html += '<ul>'; section.terminos.forEach(term => html += `<li><strong>${term.abreviatura}:</strong> ${term.definicion}</li>`); html += '</ul>'; }
                ...
                for (const [key, value] of Object.entries(sub)) {
                    ...
                    if (Array.isArray(value)) {
                        ...
                        for (const [subKey, subValue] of Object.entries(value)) {
                            ...
                            } else { html += `<li><em>${subKey}:</em> ${subValue}</li>`; }
                        }
                        html += `</ul>`;
                    }
                }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (generateButton) {
        console.log("Botón 'Generar Protocolo' encontrado. Asociando evento de clic.");
        // Ahora es seguro asociar el evento porque todas las funciones y datos están definidos.
        generateButton.addEventListener('click', generateProtocol);
    } else {
        console.error("Error CRÍTICO: No se encontró el botón con id='generateButton'. La aplicación no puede funcionar.");
        alert("Error de inicialización. No se encontró el botón de generar. Revise la consola.");
        return;
    }

    // Cargar el último protocolo guardado en Local Storage
    try {
        const savedProtocol = localStorage.getItem('lastGeneratedProtocol');
        if (savedProtocol) {
            console.log("Se encontró un protocolo guardado. Renderizando...");
        ...
        localStorage.removeItem('lastGeneratedProtocol'); // Limpiar si está corrupto
    }
});
